...
export default {
  ...
  methods: {
    submitForm() {
      ...
      this.$store.dispatch('users/login', { email, password }).then(() => {
        this.$router.push({ name: 'index' })
        this.loading = false
      }).catch((error) => {
        this.loading = false
        this.$store.commit('users/notification', { type: 'error', message: error.message })
      })
    },
    toggleShowPassword() {
    ...
    recover() {
      if (!this.user.email) {
        this.$store.commit('users/notification', { type: 'warning', message: 'Informe o email usado no cadastro' })
        return false
      }
      this.$store.dispatch('users/resetPassword', this.user.email).then(() => {
        this.$store.commit('users/notification', { type: 'info', message: 'Email de redefinição enviado' })
      }).catch((error) => {
        this.$store.commit('users/notification', { type: 'error', message: error.message })
      })
    }
