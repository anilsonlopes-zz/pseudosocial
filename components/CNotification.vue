<template>
  <div class="rounded py-2 px-4 mb-4 animated bounceInRight faster" :class="className">
...
export default {
  ...
  computed: {
    className() {
      const types = {
        default: 'bg-black text-grey-light shadow',
  ...
  mounted() {
    const duration = this.notification.duration || this.duration || 3
    window.setTimeout(() => {
      this.$store.commit('users/removeNotification', this.notification.id)
    }, duration * 1000)
  }
}
</script>
